...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		v1.add("aaa");
		v1.add(111);												//자동으로 컴파일가 int형타입의 변수를 객체로 변경해서 저장해준다.
		v1.add(new Integer(123));	//오토박싱
		v1.add('a');
		v1.add(true);
		v1.add(3.14);
		...
		v1.add(1, "kkk");											//오버로드된 메서드
		System.out.println("v1 => " +v1.toString());
		
		//set(index, 데이터) => 벡터의 index번째의 값을 주어진 '데이터'로		//수정작업을 함.
		//					=> 덮어쓴다. (반환값: 원래의 데이터)
		...
		v1.remove(new Integer(123));				//객체로 만들어서  사용해야 한다.
		System.out.println("삭제 후 : " + v1);
		System.out.println();
		...
		//	만약 제너릭을 사용하지 않으 Collection을 향상된 for문으로 처리할
		// 경우에는 Object형으로 처리한다.
		for(Object obj : v1) {
		...
		Vector v =new Vector<>(5); //용량이 5(사이즈는 0) 벡터 생성
		v.add("홍길동");
		v.add("박찬호");
		v.add("3");
		print(v);
		...
		v.setSize(7);			//현재용량이 설정사이즈보다 작으면 신규용량 = 현재용량 * 2
