...
public class T03_EnumTest {
	//City 열거형 객체 선언(기본값을 이용하여 열거형)
	public enum City { 서울, 부산, 대구, 광주, 대전};	//--------------각각의 City타입의 객체들
	...
	public enum Season{
		...
		// 값을 반환하는 메서드 작성
		public String getStr() {			//geter생성으로 언제든 str을 가져와 쓸수있다.
			return str;
		}
	}
	public static void main(String[] args) {
		 ...
		 * 3. valueof("열거형상수이름"); => 지정된 열거형에서 '열거형상수이름'과 일치
		 * 						하는 열거형 상수를 반환한다.
		 */
			...
			System.out.println("name => " + ss.name());			//객체이름
			System.out.println("ordinal => " + ss.ordinal());	//객체위치
			System.out.println("get메서드 => " + ss.getStr());		//객체의 값
